	...
	def getAtWarCivilizations(self, iPlayer):
		...
		for pyPlayer in playerList:
			...
			if(playerTeam.isAtWar(cyPlayer.getTeam())):
				...
				if ((cyPlayer.getID() < con.iNumPlayers and gc.getTeam(gc.getPlayer(iPlayer).getTeam()).canContact(cyPlayer.getID()))): #RFGW
					enemyPlayersList.append(cyPlayer)
				#Rhye - end
...
class Mercenary:
	...
	#Rhye - start continents
	def canHireUnit(self, iPlayer): #bluepotato TODO: I removed the previous implementation since it required civ groups and those weren't actually a thing. Will re-add it at a later time
		return False
	#Rhye - end
