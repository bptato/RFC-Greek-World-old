...
##		     and iUnitType != con.un('settler') and iUnitType != con.iMechanizedInfantry):
##			return True
		if (iUnitType <= con.un('war_elephant') and iUnitType != con.un('settler')):
			return True
		if (iUnitType == con.un('catapult')):
			if (gc.getPlayer(unit.getOwner()).isHuman()):
				return True
			else:
				if (gc.getGame().getSorenRandNum(100, 'random modifier') >= 30):
					return True
		if (iUnitType == con.un('settler')):
			if (gc.getPlayer(unit.getOwner()).isHuman()):
				if (gc.getGame().getSorenRandNum(100, 'random modifier') >= 50):
					return True
			else:
	...
	def isDefenderUnit(self, unit):
		iUnitType = unit.getUnitType()
		if (iUnitType >= con.un('spearman') and iUnitType <= con.un('babylon_bowman')):
			return True
		return False
	...
	def makeUnit(self, iUnit, iPlayer, tCoords, iNum): #by LOQ
		...
		print "Making (" + str(iNum) + ") units " + str(iUnit) + " at " + str(tCoords)
		unitInfoAmount = con.getNumUnits()
		if iUnit > unitInfoAmount:
			print ("makeUnit: whaaat?! Unit id " + str(iUnit) + "!!!")
			return
		for i in range(iNum):
...
##				return iCiv
	...
	def flipUnitsInCityBefore(self, tCityPlot, iNewOwner, iOldOwner):
		...
		for i in range(iNumUnitsInAPlot):
			...
			if (unit.getOwner() == iOldOwner):
				...
				if (iNewOwner < con.iNumActivePlayers or unitType > con.un('settler')):
					self.makeUnit(unitType, iNewOwner, [0, CyMap().getGridHeight()-1], 1)
			else:
				j += 1
	...
	def flipUnitsInCityAfter(self, tCityPlot, iCiv):
		...
		tempPlot = gc.getMap().plot(0,CyMap().getGridHeight()-1)
		...
		gc.getMap().plot(2, CyMap().getGridHeight()-3).setRevealed(iCiv, False, True, -1);
		gc.getMap().plot(1, CyMap().getGridHeight()-3).setRevealed(iCiv, False, True, -1);
		gc.getMap().plot(0, CyMap().getGridHeight()-3).setRevealed(iCiv, False, True, -1);
		...
		gc.getMap().plot(CyMap().getGridWidth()-2, CyMap().getGridHeight()-3).setRevealed(iCiv, False, True, -1);
		gc.getMap().plot(CyMap().getGridWidth()-1, CyMap().getGridHeight()-3).setRevealed(iCiv, False, True, -1);
	...
	def flipUnitsInArea(self, tTopLeft, tBottomRight, iNewOwner, iOldOwner, bSkipPlotCity, bKillSettlers):
		"""Deletes, recreates units in 0,CyMap().getGridHeight()-1 and moves them to the previous tile.
		...
		for x in range(tTopLeft[0], tBottomRight[0]+1):
			for y in range(tTopLeft[1], tBottomRight[1]+1):
				...
				if (iNumUnitsInAPlot):
					bRevealedZero = False
					if (gc.getMap().plot(0, CyMap().getGridHeight()-1).isRevealed(iNewOwner, False)):
						bRevealedZero = True
					#print ("killplot", x, y)
					if (bSkipPlotCity == True) and (killPlot.isCity()):
						...
						for i in range(iNumUnitsInAPlot):
							...
							if (unit.getOwner() == iOldOwner):
								unit.kill(False, con.iBarbarian)
								if (bKillSettlers):
									if ((unit.getUnitType() > con.un('settler'))):
										self.makeUnit(unit.getUnitType(), iNewOwner, [0, CyMap().getGridHeight()-1], 1)
								else:
									if ((unit.getUnitType() >= con.un('settler'))): #skip animals
										self.makeUnit(unit.getUnitType(), iNewOwner, [0, CyMap().getGridHeight()-1], 1)
							else:
								j += 1
						tempPlot = gc.getMap().plot(0,CyMap().getGridHeight()-1)
...
##				pCurrent.setCulture(iCiv, iCivCulture + iLoopCivCulture*iPercent/100, True)
	...
	def killCiv(self, iCiv, iNewCiv):
		...
		self.flipUnitsInArea([0,0], [CyMap().getGridWidth()-1,CyMap().getGridHeight()-1], iNewCiv, iCiv, False, True)
		#self.killUnitsInArea([0,0], [CyMap().getGridWidth()-1,CyMap().getGridHeight()-1], iNewCiv, iCiv) ?
		#RFGW
	...
	def killAndFragmentCiv(self, iCiv, iNewCiv1, iNewCiv2, iNewCiv3, bAssignOneCity):
		...
		if (bAssignOneCity == False):
			#self.flipUnitsInArea([0,0], [CyMap().getGridWidth()-1,CyMap().getGridHeight()-1], iNewCiv1, iCiv, False, True) #causes a bug: if a unit was inside another city's civ, when it becomes independent or barbarian, may raze it
			self.killUnitsInArea([0,0], [CyMap().getGridWidth()-1,CyMap().getGridHeight()-1], iCiv)
			self.resetUHV(iCiv)
		#RFGW
		#if (iCiv < iNumMajorPlayers):
		#	self.clearEmbassies(iCiv)
	...
	def resetUHV(self, iPlayer):
		print "reset uhv for " + str(iPlayer)
	...
	def clearPlague(self, iCiv):
		for pyCity in PyPlayer(iCiv).getCityList():
			if (pyCity.GetCy().hasBuilding(con.bn("plague"))):
				pyCity.GetCy().setHasRealBuilding(con.bn("plague"), False)
	...
	def outerInvasion(self, tCoords, result, argsList):
		...
		bContinue = True
		pCurrent = gc.getMap().plot(tCoords[0], tCoords[1])
		if (pCurrent.isHills() or pCurrent.isFlatlands() and pCurrent.area().getNumTiles() >= 9):
			if (pCurrent.getTerrainType() != con.iMarsh and pCurrent.getTerrainType() != con.iSnow and pCurrent.getTerrainType() != con.iTundra) and (pCurrent.getFeatureType() != con.iJungle): #RFCRAND
	...
	def outerSpawn(self, tCoords, result, argsList):
		...
		if (pCurrent.isHills() or pCurrent.isFlatlands()):
			if (pCurrent.getTerrainType() != con.iSnow and pCurrent.getTerrainType() != con.iTundra) and (pCurrent.getTerrainType() != con.iMarsh) and (pCurrent.getFeatureType() != con.iJungle): #RFGW
				if (not pCurrent.isCity() and not pCurrent.isUnit()):
					iClean = 0
					for x in range(tCoords[0] - 1, tCoords[0] + 2):	# from x-1 to x+1
						for y in range(tCoords[1] - 1, tCoords[1] + 2):	# from y-1 to y+1
	...
	def inTerritoryInvasion(self, tCoords, result, argsList):
				"""Checks validity of the plot at the current tCoords, returns plot if valid (which stops the search).
				Plot is valid if it's hill or flatlands, it isn't occupied by a unit or city"""
				bPaint = True
				bContinue = True
				pCurrent = gc.getMap().plot(tCoords[0], tCoords[1])
				if (pCurrent.isHills() or pCurrent.isFlatlands()):
	...
	def innerSpawn(self, tCoords, result, argsList):
		...
		pCurrent = gc.getMap().plot(tCoords[0], tCoords[1])
		if (pCurrent.isHills() or pCurrent.isFlatlands()):
			if (pCurrent.getTerrainType() != con.iMarsh and pCurrent.getTerrainType() != con.iSnow and pCurrent.getTerrainType() != con.iTundra) and (pCurrent.getFeatureType() != con.iJungle):
				if (not pCurrent.isCity() and not pCurrent.isUnit()):
					iClean = 0
					for x in range(tCoords[0] - 1, tCoords[0] + 2):	# from x-1 to x+1
						for y in range(tCoords[1] - 1, tCoords[1] + 2):	# from y-1 to y+1
	...
	def getScenario(self):
		if gc.getPlayer(con.iSumeria).isPlayable(): return con.i4400BCScenario

		if gc.getPlayer(con.iAssyria).isPlayable(): return con.i896BCScenario

	def getScenarioStartYear(self):
		lStartYears = [-4400, -896]
		return lStartYears[self.getScenario()]

	def getScenarioStartTurn(self):
		lStartTurn = [con.i4400BC, con.i896BC]
