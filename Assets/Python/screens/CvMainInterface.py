...
##			screen.hide(szName)
		...
		screen.addStackedBarGFC("ReligionBar", 16, yResolution - 214 - iStackBarHeight, 220, iStackBarHeight, InfoBarTypes.NUM_INFOBAR_TYPES, WidgetTypes.WIDGET_HELP_BELIEVERS, -1, -1)
		screen.hide("ReligionBar")
	...
	def updateMiscButtons(self):
		...
		if (CyInterface().getShowInterface() == InterfaceVisibility.INTERFACE_HIDE_ALL or CyInterface().getShowInterface() == InterfaceVisibility.INTERFACE_MINIMAP_ONLY):
			...
			screen.hide("CivicsAdvisorButton")
			screen.hide("ReligiousAdvisorButton")
			#screen.hide("CorporationAdvisorButton") #Rhye RFGW
			screen.hide("FinanceAdvisorButton")
			screen.hide("MilitaryAdvisorButton")
			screen.hide("VictoryAdvisorButton")
			screen.hide("InfoAdvisorButton")
			...
			#screen.show("CorporationAdvisorButton")#Rhye RFGW
			...
			#screen.moveToFront("CorporationAdvisorButton")#Rhye RFGW
...
##					screen.show(szName)
				...
				religionType = "No state religion"
				religionPercentage = 0
				if gc.getPlayer(pHeadSelectedCity.getOwner()).getStateReligion() != -1:
					religionType = localText.getText(str(gc.getReligionInfo(gc.getPlayer(pHeadSelectedCity.getOwner()).getStateReligion()).getDescription()), ())
					religionPercentage = int(float(pHeadSelectedCity.getBelievers(gc.getPlayer(pHeadSelectedCity.getOwner()).getStateReligion())) / float(pHeadSelectedCity.getPopulation()) * 100)

				print "believers:"
				print str(pHeadSelectedCity.getBelievers(gc.getPlayer(pHeadSelectedCity.getOwner()).getStateReligion()))
				print str(pHeadSelectedCity.getPopulation())
				for i in range(gc.getNumReligionInfos()):
					print "all: " + str(pHeadSelectedCity.getBelievers(i))

				szBuffer = u"%d%% %s" %(religionPercentage, religionType)
				screen.setLabel("BelieverText", "Background", szBuffer, CvUtil.FONT_CENTER_JUSTIFY, 125, yResolution - 210 - iStackBarHeight, -0.3, FontTypes.SMALL_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1)
				screen.setHitTest("BelieverText", HitTestTypes.HITTEST_NOHIT)
				screen.show("BelieverText")
				iWhichBar = 0
				iRemainder = 100
				for i in range(gc.getNumReligionInfos()):
					iPercent = int(float(pHeadSelectedCity.getBelievers(i)) / float(pHeadSelectedCity.getPopulation()) * 100)
					if (iPercent > 0):
						screen.setStackedBarColorsRGB("ReligionBar", iWhichBar, gc.getPlayer(i).getPlayerTextColorR(), gc.getPlayer(i).getPlayerTextColorG(), gc.getPlayer(i).getPlayerTextColorB(), gc.getPlayer(i).getPlayerTextColorA())
						if (iRemainder <= 0):
							screen.setBarPercentage("ReligionBar", iWhichBar, 0.0)
						else:
							screen.setBarPercentage("ReligionBar", iWhichBar, float(iPercent) / iRemainder)
						iRemainder -= iPercent
						iWhichBar += 1
				screen.show("ReligionBar")
