...
bool CvUnit::canEnterTerritory(TeamTypes eTeam, bool bIgnoreRightOfPassage) const
{
	...
	//Rhye - start UP (Dutch)
	if (getOwnerINLINE() == PHOENICIA && DOMAIN_SEA == getDomainType())
		return true;
	//Rhye - end
...
int CvUnit::workRate(bool bMax) const
{
	...
	//Rhye - start switch
	//RFGW
	int iStartingTurn = GET_PLAYER(getOwner()).getStartingTurn();
	...
	CyArgsList argsList;
	argsList.add(getOwner());
	gDLL->getPythonIFace()->callFunction(PYScreensModule, "getWorkRateModifier", argsList.makeFunctionArgs(), &result);
	int iResult = (int)result;

	iRate = iRate*iResult/100;
	//Rhye - end
