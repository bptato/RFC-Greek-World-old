...
//Speed: End Modify
		...
		if (lResult == 1)
		{
			...
			CyArgsList argsList;
			argsList.add(getID());
			gDLL->getPythonIFace()->callFunction(PYScreensModule, "getRazeOnly", argsList.makeFunctionArgs(), &result);
	...
	for (iI = 0; iI < GC.getNumBuildingClassInfos(); iI++)
	{
		...
		if (eLoopBuilding != NO_BUILDING)
		{
			...
			if (GC.getBuildingInfo(eLoopBuilding).getFreeStartEra() != NO_ERA) //Rhye
			{
				...
				CyArgsList argsList;
				argsList.add(getID());
				gDLL->getPythonIFace()->callFunction(PYScreensModule, "getCitiesStartEraModifier", argsList.makeFunctionArgs(), &result);

				if (getID()<NUM_MAJOR_PLAYERS)
					startingEra = (int)result;
...
int CvPlayer::getProductionNeeded(UnitTypes eUnit) const
{
	...
	//Rhye - start switch
	//RFGW
	int iStartingTurn = GET_PLAYER(getID()).getStartingTurn();

	if (getID()<NUM_MAJOR_PLAYERS)
		iProductionNeeded += std::max(-50,(iProductionNeeded*(0 - iStartingTurn)/BASIC_TURN_MODIFIER));
...
int CvPlayer::getProductionNeeded(BuildingTypes eBuilding) const
{
	...
	CyArgsList argsList;
	argsList.add(getID());
	gDLL->getPythonIFace()->callFunction(PYScreensModule, "getWonderProductionModifier", argsList.makeFunctionArgs(), &result);
	int iResult = (int)result;
	...
	CyArgsList argsList;
	argsList.add(getID());
	gDLL->getPythonIFace()->callFunction(PYScreensModule, "getBuildingProductionModifier", argsList.makeFunctionArgs(), &result);
	int iResult = (int)result;
...
int CvPlayer::calculateInflationRate() const
{
	...
	if (getID()<NUM_MAJOR_PLAYERS)
		iRate += std::max(-50,(iRate*(0 - iStartingTurn)/BASIC_TURN_MODIFIER));
	long result = -1;
	CyArgsList argsList;
	argsList.add(getID());
	gDLL->getPythonIFace()->callFunction(PYScreensModule, "getInflationModifier", argsList.makeFunctionArgs(), &result);
	int iResult = (int)result;

	if (getID()<NUM_MAJOR_PLAYERS)
		iRate = iRate*result/100;
...
int CvPlayer::greatPeopleThreshold(bool bMilitary) const
{
	...
	if (getID()<NUM_MAJOR_PLAYERS)
		iThreshold += std::max(-50,(iThreshold*(0 - iStartingTurn)/BASIC_TURN_MODIFIER));

	long result = -1;
	CyArgsList argsList;
	argsList.add(getID());
	gDLL->getPythonIFace()->callFunction(PYScreensModule, "getGreatPeopleModifier", argsList.makeFunctionArgs(), &result);
	int iResult = (int)result;

	if (getID()<NUM_MAJOR_PLAYERS)
		iThreshold = iThreshold*result/100;
...
int CvPlayer::verifySettlersHalt(int threshold)
{
	...
	for (iI = 0; iI < GC.getMapINLINE().getGridWidthINLINE(); iI++) //RFGW
	{
		for (iJ = 0; iJ < GC.getMapINLINE().getGridHeightINLINE(); iJ++) //RFGW
		{
			...
			if (pLoopPlot != NULL)
			{
				if (canFound(iI, iJ))
				{
					...
					{
						//Rhye RFGW
						//if (settlersMaps[getID()][GC.getMapINLINE().getGridHeightINLINE() -1 -iJ][iI] >= threshold)
						if (getSettlersMaps(GC.getMapINLINE().getGridHeightINLINE() -1 -iJ,iI) >= threshold) //RFGW
						{
							count++;
...
int CvPlayer::getStartingTurn() {
	long result = -1;
	CyArgsList argsList;
	argsList.add(getID());
	gDLL->getPythonIFace()->callFunction(PYScreensModule, "getStartingTurn", argsList.makeFunctionArgs(), &result);
	return (int)result;
}
...
int CvPlayer::getSettlersMaps(int y, int x)
{
	...
	argsList.add(y);
	argsList.add(x);
	gDLL->getPythonIFace()->callFunction(PYScreensModule, "getSettlersMaps", argsList.makeFunctionArgs(), &result);
	int iSettlersMaps = (int)result;

	//Rhye RFGW debug
	if (x >= GC.getMapINLINE().getGridWidthINLINE() || y >= GC.getMapINLINE().getGridHeightINLINE() || x < 0 || y < 0) {
		char buf[30];
		sprintf(buf, "out of bounds - civ: %d; x,y: %d, %d ", getID(), x,y);
		GC.getGameINLINE().logMsg(buf);
	}
	return iSettlersMaps;
}
...
int CvPlayer::getGrowthThreshold(int iPopulation) const
{
	...
	CyArgsList argsList;
	argsList.add(getID());
	gDLL->getPythonIFace()->callFunction(PYScreensModule, "getGrowthModifier", argsList.makeFunctionArgs(), &result);
	int iResult = (int)result;
