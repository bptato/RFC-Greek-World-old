...
void CvPlayerAI::AI_updateFoundValues(bool bStartingLoc) const
{
	...
	//GC.getGameINLINE().logMsg("AI_updateFoundValues"); //Rhye RFGW
	...
	else
	{
		...
		for (iI = 0; iI < GC.getMapINLINE().numPlotsINLINE(); iI++)
		{
			...
			if (pLoopPlot->isRevealed(getTeam(), false))// || AI_isPrimaryArea(pLoopPlot->area()))
			{
				...
				//Rhye RFGW debug
				/*char buf[30];
				sprintf(buf, "X,Y: %d, %d; iValue: %d;", pLoopPlot->getX(),pLoopPlot->getY(),iValue);
...
void CvPlayerAI::AI_conquerCity(CvCity* pCity)
{
	...
	if (!bRaze)
	{
		CvEventReporter::getInstance().cityAcquiredAndKept(pCity->getPreviousOwner(), GC.getGameINLINE().getActivePlayer(), pCity, true, false);
	}
}
...
int CvPlayerAI::AI_foundValue(int iX, int iY, int iMinRivalRange, bool bStartingLoc) const
{
	...
	CyArgsList argsList;
	argsList.add(getID());
	gDLL->getPythonIFace()->callFunction(PYScreensModule, "getCityClosenessModifier", argsList.makeFunctionArgs(), &result);
	int iResult = (int)result;

	if (iTakenTiles > iResult)
			return 0;
		...
		else
		{
			...
			*/
			...
			else if (getCapitalCity() != NULL)
			{
				...
				int compactEmpireModifier = (int)result;
	...
	//Rhye RFGW
	//iValue *= settlersMaps[getID()][GC.getMapINLINE().getGridHeightINLINE() - 1 - tempY][tempX];
	iValue *= GET_PLAYER(getID()).getSettlersMaps(GC.getMapINLINE().getGridHeightINLINE() - 1 - tempY,tempX);
	iValue /= 100;

	if (GET_PLAYER(getID()).getSettlersMaps(GC.getMapINLINE().getGridHeightINLINE() - 1 - tempY,tempX) <= 3)
		iValue /= 2;
	if (GET_PLAYER(getID()).getSettlersMaps(GC.getMapINLINE().getGridHeightINLINE() - 1 - tempY,tempX) <= 60)
		iValue /= 2;
	/*if (GET_PLAYER(getID()).getSettlersMaps(GC.getMapINLINE().getGridHeightINLINE() - 1 - tempY,tempX) >= 200) {
		iValue *= 11;
		iValue /= 10;
	}
	if (GET_PLAYER(getID()).getSettlersMaps(GC.getMapINLINE().getGridHeightINLINE() - 1 - tempY,tempX) >= 400) {
		iValue *= 11;
		iValue /= 10;
	}*/
	if (GET_PLAYER(getID()).getSettlersMaps(GC.getMapINLINE().getGridHeightINLINE() - 1 - tempY,tempX) > 60)
	//	iValue += GC.getGameINLINE().getSorenRandNum(1000, "Random Value");
	{
		...
		iValue *= (100 + ((GC.getGameINLINE().getSorenRand().getSeed() % ((tempX+10)*(tempY+10) + (GC.getMapINLINE().getGridWidthINLINE()-tempX) + (GC.getMapINLINE().getGridHeightINLINE()-tempY))) % 50)); //RFCMP //RFGW
		iValue /= 100;
	}
	//Rhye - end
...
int CvPlayerAI::AI_targetCityValue(CvCity* pCity, bool bRandomize, bool bIgnoreAttackers) const
{
	...
	//Rhye - start
	//RFGW settlersMaps[getID()][GC.getMapINLINE().getGridHeightINLINE() - 1 - pCity->plot()->getY_INLINE()][pCity->plot()->getX_INLINE()]
	if (GET_PLAYER(getID()).getSettlersMaps(GC.getMapINLINE().getGridHeightINLINE() - 1 - pCity->plot()->getY_INLINE(),pCity->plot()->getX_INLINE()) <= 3)
		iValue -= 2;
	else if (GET_PLAYER(getID()).getSettlersMaps(GC.getMapINLINE().getGridHeightINLINE() - 1 - pCity->plot()->getY_INLINE(),pCity->plot()->getX_INLINE()) <= 20)
		iValue -= 1;
	else if (GET_PLAYER(getID()).getSettlersMaps(GC.getMapINLINE().getGridHeightINLINE() - 1 - pCity->plot()->getY_INLINE(),pCity->plot()->getX_INLINE()) >= 500) //500-700
		iValue += 4;
	else if (GET_PLAYER(getID()).getSettlersMaps(GC.getMapINLINE().getGridHeightINLINE() - 1 - pCity->plot()->getY_INLINE(),pCity->plot()->getX_INLINE()) >= 300) //300-400
		iValue += 3;
	else if (GET_PLAYER(getID()).getSettlersMaps(GC.getMapINLINE().getGridHeightINLINE() - 1 - pCity->plot()->getY_INLINE(),pCity->plot()->getX_INLINE()) >= 150) //150-200
		iValue += 2;
	else if (GET_PLAYER(getID()).getSettlersMaps(GC.getMapINLINE().getGridHeightINLINE() - 1 - pCity->plot()->getY_INLINE(),pCity->plot()->getX_INLINE()) >= 40) //40-60-90
		iValue += 1;
	...
	CyArgsList argsList;
	argsList.add(getID());
	gDLL->getPythonIFace()->callFunction(PYScreensModule, "getFeedUnits", argsList.makeFunctionArgs(), &result);
	int iResult = (int)result;
	if (iResult != 0) {
	...
	if (pNearestCity != NULL)
	{
		...
		*/
		...
		CyArgsList argsList;
		argsList.add(getID());
		gDLL->getPythonIFace()->callFunction(PYScreensModule, "getWarDistanceModifier", argsList.makeFunctionArgs(), &result);
		int iResult = (int)result;

		iValue += std::max(1, ((GC.getMapINLINE().maxStepDistance() * 2) - GC.getMapINLINE().calculatePathDistance(pNearestCity->plot(), pCity->plot()))*iResult/3);
...
bool CvPlayerAI::AI_isFinancialTrouble() const
{
	//if (getCommercePercent(COMMERCE_GOLD) > 50)
	{
		...
		argsList2.add(getID());
		gDLL->getPythonIFace()->callFunction(PYScreensModule, "getStartingTurn", argsList2.makeFunctionArgs(), &result);
		iResult = (int)result;
		if (GC.getGameINLINE().getGameTurn() <= iResult + 2)
			return false;
		//Rhye - end
...
int CvPlayerAI::AI_getAttitudeVal(PlayerTypes ePlayer, bool bForced) const
{
	...
	//Rhye - start UP
	if (ePlayer == HITTITES) {
		if ((getID() != EGYPT) && (getID() != PHOENICIA)
			&& (getID() != BABYLONIA) && (getID() != MYCENAE) && (getID() != ASSYRIA) && (getID() != ISRAEL)
			&& (getID() != SUMERIA)
			&& (getID() != ATHENS) && (getID() != SPARTA))
			iAttitude += 8;
	}
	//Rhye - end UP
...
int CvPlayerAI::AI_cityTradeVal(CvCity* pCity) const
{
	...
	if (GET_PLAYER(pCity->getOwnerINLINE()).getNumCities() > 12)
			//Rhye RFGW
		//if (settlersMaps[pCity->getOwnerINLINE()][GC.getMapINLINE().getGridHeightINLINE() -1 - pCity->getY()][pCity->getX()] < 500) { //not in core area
		if (GET_PLAYER(pCity->getOwnerINLINE()).getSettlersMaps(GC.getMapINLINE().getGridHeightINLINE() -1 - pCity->getY(),pCity->getX()) < 500) { //not in core area
			iValue *= 2;
			iValue /= 3;
		}
	//Rhye - end
...
int CvPlayerAI::AI_civicValue(CivicTypes eCivic) const
{
	...
	if (GC.getCivicInfo(eCivic).isStabilityFoundBonus()) {
		...
		CyArgsList argsList;
		argsList.add(getID());
		gDLL->getPythonIFace()->callFunction(PYScreensModule, "getStabilityFoundModifier", argsList.makeFunctionArgs(), &result);
		int iResult = (int)result;
	...
	if (GC.getCivicInfo(eCivic).isStabilityConquestBonus()) {
		...
		CyArgsList argsList;
		argsList.add(getID());
		gDLL->getPythonIFace()->callFunction(PYScreensModule, "getStabilityConquestModifier", argsList.makeFunctionArgs(), &result);
		int iResult = (int)result;
	...
	if (GC.getCivicInfo(eCivic).isStabilityCommerceBonus()) {
		...
		CyArgsList argsList;
		argsList.add(getID());
		gDLL->getPythonIFace()->callFunction(PYScreensModule, "getStabilityCommerceModifier", argsList.makeFunctionArgs(), &result);
		int iResult = (int)result;
...
void CvPlayerAI::AI_updateCitySites(int iMinFoundValueThreshold, int iMaxSites) const
{
	...
	while (iPass < iMaxSites)
	{
		...
		for (iI = 0; iI < GC.getMapINLINE().numPlotsINLINE(); iI++)
		{
			...
			{
				//Rhye RFGW
				//if (settlersMaps[getID()][GC.getMapINLINE().getGridHeightINLINE() -1 - pLoopPlot->getY()][pLoopPlot->getX()] >= 90) //Rhye
				if (GET_PLAYER(getID()).getSettlersMaps(GC.getMapINLINE().getGridHeightINLINE() -1 - pLoopPlot->getY(),pLoopPlot->getX()) >= 90) //Rhye
				{ //Rhye
...
{/*
	...
	for (iI = 0; iI < GC.getMapINLINE().getGridWidthINLINE(); iI++)
	{
	for (iJ = 0; iJ < GC.getMapINLINE().getGridHeightINLINE(); iJ++)
	{
