...
void CvCity::init(int iID, PlayerTypes eOwner, int iX, int iY, bool bBumpUnits, bool bUpdatePlotGroups)
{
	...
	argsList2.add(getOwnerINLINE());
	gDLL->getPythonIFace()->callFunction(PYScreensModule, "getCitiesStartEraModifier", argsList2.makeFunctionArgs(), &result);

	eraModifier = (int)result;
...
void CvCity::doTask(TaskTypes eTask, int iData1, int iData2, bool bOption, bool bAlt, bool bShift, bool bCtrl)
{
	int loopI;
	int sackGold;
	PlayerTypes ownerType = getOwnerINLINE();
	CvPlayerAI& owner = GET_PLAYER(getOwnerINLINE());
	...
	case TASK_SACK:
		...
		for(int i = 0; i<GC.getNumBuildingInfos(); i++) {
			if(hasBuilding((BuildingTypes)i) && GC.getGameINLINE().getSorenRandNum(100, "Loot chance") <= 90) {
                sackGold += GC.getGameINLINE().getSorenRandNum(getBuildingProduction((BuildingTypes) i)/10, "Sack gold 1") + GC.getGameINLINE().getSorenRandNum(getBuildingProduction((BuildingTypes) i)/10, "Sack gold 2");

				setHasRealBuilding((BuildingTypes) i, false);
			}
		}
		owner.changeGold(sackGold);
		if (getPreviousOwner() != NO_PLAYER && GET_PLAYER(getPreviousOwner()).isAlive())
		{
			GET_PLAYER(getPreviousOwner()).acquireCity(this, false, false, false);
			for (CvUnit* loopUnit = owner.firstUnit(&loopI); loopUnit != NULL; loopUnit = owner.nextUnit(&loopI)) {
				if(loopUnit->getX()==getX() && loopUnit->getY()==getY()) {
					//loopUnit->kill();
					loopUnit->changeDamage(loopUnit->currHitPoints()/10, ownerType);
					loopUnit->jumpToNearestValidPlot();
				}
			}
			//GET_PLAYER((PlayerTypes)iData1).acquireCity(this, false, false, false);
			//setOccupationTimer(occupationTimer + 1);
		}
		else
		{
			GET_PLAYER(getOwnerINLINE()).raze(this);
...
//bluepotato start
bool CvCity::canBeSacked() {
	//TODO return (getNumBuildings() >= 5 || GET_PLAYER(getPreviousOwner()).getPreviousCapitalCity() == this || GET_PLAYER(getPreviousOwner()).getCapitalCity() == this;
	return getNumBuildings() > 0;
}
//bluepotato end
	...
	if (!isOccupation())
	{
		for (int iI = (GC.getNumCultureLevelInfos() - 1); iI > 0; iI--)
		{
			if (getCultureTimes100(getOwnerINLINE()) >= 100 * GC.getGameINLINE().getCultureThreshold((CultureLevelTypes)iI) ||
				(getOwner() == SASSANID && getCultureTimes100(getOwnerINLINE()) >= 75 * GC.getGameINLINE().getCultureThreshold((CultureLevelTypes)iI)))
			{
  ...
  if (pOrderNode == NULL)
  {
	...
	case ORDER_CONSTRUCT:
		...
		if (bFinish)
		{
			...
			if(getOwnerINLINE()==ELAM && getBuildingCommerceChange((BuildingClassTypes)(GC.getBuildingInfo(eConstructBuilding).getBuildingClassType()), COMMERCE_CULTURE) > 0) {
...
//Speed: End Modify
	...
	if 	(getCommerceRate(COMMERCE_CULTURE) <=4)
		...
		//RFGW
		int iStartingTurn = GET_PLAYER(getOwnerINLINE()).getStartingTurn();

		int iModifier = 100;
		iModifier += std::max(-50,(iModifier*(0 - iStartingTurn)/BASIC_TURN_MODIFIER));

		long result = -1;
		CyArgsList argsList;
		argsList.add(getOwnerINLINE());
		gDLL->getPythonIFace()->callFunction(PYScreensModule, "getCommerceGrowthModifier", argsList.makeFunctionArgs(), &result);
		int iResult = (int)result;

		iModifier = iModifier*iResult/100;
		changeCultureTimes100(getOwnerINLINE(), getCommerceRateTimes100(COMMERCE_CULTURE) *iModifier /100, false, true);
