...
//	}
	...
	for (int iI = 0; iI < GET_PLAYER(getOwnerINLINE()).AI_getNumCitySites(); iI++)
	{
		...
		//Rhye - start
		//Rhye RFGW
		//if (settlersMaps[getOwnerINLINE()][GC.getMapINLINE().getGridHeightINLINE() -1 -pCitySitePlot->getY_INLINE()][pCitySitePlot->getX_INLINE()] < 150) //so high?
		if (GET_PLAYER(getOwnerINLINE()).getSettlersMaps(GC.getMapINLINE().getGridHeightINLINE() -1 -pCitySitePlot->getY_INLINE(),pCitySitePlot->getX_INLINE()) < 150) 
			return false;
		//Rhye - end
...
bool CvUnitAI::AI_found_map(int modifier)
{
	...
	for (iI = 0; iI < GC.getMapINLINE().getGridWidthINLINE(); iI++) //RFGW
	{
		for (iJ = 0; iJ < GC.getMapINLINE().getGridHeightINLINE(); iJ++)
		{
			...
			//Rhye RFGW
			//if (settlersMaps[getOwnerINLINE()][GC.getMapINLINE().getGridHeightINLINE() -1 -iJ][iI] == modifier)
			if (GET_PLAYER(getOwnerINLINE()).getSettlersMaps(GC.getMapINLINE().getGridHeightINLINE() -1 -iJ,iI) == modifier)
			{
				if (pLoopPlot != NULL)
				{
					if (AI_plotValid(pLoopPlot))
		...
		for (iDY = -(iSearchRange); iDY <= iSearchRange; iDY++)
		{
			...
			if (pLoopPlot != NULL)
			{
				if (AI_plotValid(pLoopPlot) && (pLoopPlot != plot() || GET_PLAYER(getOwnerINLINE()).AI_getPlotDanger(pLoopPlot, 1) <= pLoopPlot->plotCount(PUF_canDefend, -1, -1, getOwnerINLINE())))
				{
					...
					{
						//Rhye RFGW
						//if (getOwnerINLINE() > NUM_MAJOR_PLAYERS || settlersMaps[getOwnerINLINE()][GC.getMapINLINE().getGridHeightINLINE() -1 - (iDY+getY_INLINE())][(iDX+getX_INLINE())] >= 60) //Rhye
						if (getOwnerINLINE() > NUM_MAJOR_PLAYERS || GET_PLAYER(getOwnerINLINE()).getSettlersMaps(GC.getMapINLINE().getGridHeightINLINE() -1 - (iDY+getY_INLINE()),(iDX+getX_INLINE())) >= 60) //Rhye
						{ //Rhye
