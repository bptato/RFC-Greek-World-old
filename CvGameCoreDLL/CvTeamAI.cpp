...
DenialTypes CvTeamAI::AI_surrenderTrade(TeamTypes eTeam, int iPowerMultiplier) const
{
	...
	if (GC.getGameINLINE().getGameTurn() <= iResult+5)
	{
		iMasterPower /= 2;
	}
	//Rhye - end
...
DenialTypes CvTeamAI::AI_makePeaceTrade(TeamTypes ePeaceTeam, TeamTypes eTeam) const
{
	...
	CyArgsList argsList;
	argsList.add(getID());
	gDLL->getPythonIFace()->callFunction(PYScreensModule, "getRefusePeaceAI", argsList.makeFunctionArgs(), &result);
	int iResult = (int)result;
	...
	gDLL->getPythonIFace()->callFunction(PYScreensModule, "getStartingTurn", argsList2.makeFunctionArgs(), &result);
	iResult = (int)result;
	if (GC.getGameINLINE().getGameTurn() < iResult + 10)
		return DENIAL_JOKING;
	//Rhye - end
	...
	for (std::vector<PlayerTypes>::const_iterator iter = m_aePlayerMembers.begin(); iter != m_aePlayerMembers.end(); ++iter)
			{
				...
				if ((eAttitude <= GC.getLeaderHeadInfo(GET_PLAYER(*iter).getPersonalityType()).getDefensivePactRefuseAttitudeThreshold() && iGameTurn < 400) ||
