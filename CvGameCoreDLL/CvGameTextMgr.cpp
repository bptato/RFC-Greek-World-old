...
//Rhye - lots of stuff to update here - leaving wchar, as in v14
	...
	if (eCivilization != NO_CIVILIZATION)
	{
		...
		setDateStr(szText, iResult, false, (CalendarTypes)0, -4400, (GameSpeedTypes)0);
		...
		szText += gDLL->getText(GC.getCivilizationInfo(eCivilization).getUHVString() + "1");
		szText += NEWLINE L"  " + gDLL->getText("TXT_KEY_ICON_BULLET");
		szText += gDLL->getText(GC.getCivilizationInfo(eCivilization).getUHVString() + "2");
		szText += NEWLINE L"  " + gDLL->getText("TXT_KEY_ICON_BULLET");
		szText += gDLL->getText(GC.getCivilizationInfo(eCivilization).getUHVString() + "3");
		szText += NEWLINE L"  ";
...
void CvGameTextMgr::parseCivicInfo(CvWStringBuffer &szHelpText, CivicTypes eCivic, bool bCivilopediaText, bool bPlayerContext, bool bSkipName)
{
	...
	{
		//bluepotato: fix unhappiness display
		szHelpText.append(NEWLINE);
