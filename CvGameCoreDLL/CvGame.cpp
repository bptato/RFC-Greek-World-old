...
void CvGame::init(HandicapTypes eHandicap)
{
	...
	for (int i = 0; i < MAX_PLAYERS; ++i)
	{
		...
		if (kPlayer.isAlive())
		{
			...
			//logMsg("call AI_updateFoundValues");
		}
	}
	//Rhye RFGW - end
}
...
void CvGame::setInitialItems()
{
	...
	for (int i = 0; i < MAX_PLAYERS; ++i)
	{
		...
		{
			kPlayer.AI_updateFoundValues();
			//logMsg("setInitialItems2"); //Rhye RFGW
		}
	}
}
...
//Speed: End Modify
		...
		GET_PLAYER(getActivePlayer()).initUnit(((UnitTypes)25), 0, 0); //Rhye (catapult) //RFGW
