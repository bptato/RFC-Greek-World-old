...
#include "CyArgsList.h" //Rhye RFGW
...
void CvCivilizationInfo::write(FDataStreamBase* stream)
{
	...
	//bluepotato start
	stream->WriteString(m_UHVString);
	//bluepotato end
...
{
	int iPercent = m_iUnitCostPercent;
	...
	CyArgsList argsList;
	argsList.add(pl);
	gDLL->getPythonIFace()->callFunction(PYScreensModule, "getUnitCostModifier", argsList.makeFunctionArgs(), &result);
	int iResult = (int)result;

	return iPercent*iResult/100;
}
//Rhye - end
...
int CvHandicapInfo::getResearchPercentByID(PlayerTypes pl) const
{
	...
	//RFGW
	int iStartingTurn = GET_PLAYER(pl).getStartingTurn();

	if (GC.getGameINLINE().getGameTurn() >= iStartingTurn) {
		researchPercent *= humanContributionPercent;
		researchPercent /= 100;
	}

	//RFGW
	researchPercent = researchPercent*135/100;
	if (pl<NUM_MAJOR_PLAYERS)
		researchPercent += std::max(-50,(researchPercent*(0 - iStartingTurn)/BASIC_TURN_MODIFIER));

	long result = -1;
	CyArgsList argsList;
	argsList.add(pl);
	gDLL->getPythonIFace()->callFunction(PYScreensModule, "getResearchModifier", argsList.makeFunctionArgs(), &result);
	int iResult = (int)result;

	if (pl<NUM_MAJOR_PLAYERS)
		researchPercent = researchPercent*iResult/100;
...
{
	int iPercent = m_iDistanceMaintenancePercent;

	//RFGW
	long result = -1;
	CyArgsList argsList;
	argsList.add(pl);
	gDLL->getPythonIFace()->callFunction(PYScreensModule, "getDistanceMaintenanceModifier", argsList.makeFunctionArgs(), &result);
	int iResult = (int)result;
...
{
	int iPercent = m_iNumCitiesMaintenancePercent;

	//RFGW
	long result = -1;
	CyArgsList argsList;
	argsList.add(pl);
	gDLL->getPythonIFace()->callFunction(PYScreensModule, "getNumCitiesMaintenanceModifier", argsList.makeFunctionArgs(), &result);
	int iResult = (int)result;
...
{
	int iPercent = m_iCivicUpkeepPercent;

	//RFGW
	long result = -1;
	CyArgsList argsList;
	argsList.add(pl);
	gDLL->getPythonIFace()->callFunction(PYScreensModule, "getCivicUpkeepModifier", argsList.makeFunctionArgs(), &result);
	int iResult = (int)result;
...
{
	int iBonus = m_iHealthBonus;

	//RFGW
	long result = -1;
	CyArgsList argsList;
	argsList.add(pl);
	gDLL->getPythonIFace()->callFunction(PYScreensModule, "getHealthBonusModifier", argsList.makeFunctionArgs(), &result);
	int iResult = (int)result;
